...
class Pixiv {
	...
	AnauthGot(path, opt) {
		return new Promise((resolve, reject) => {
			this._auth().then(() => {
				...
				//console.log(url.resolve('https://public-api.secure.pixiv.net/v1/', path) + '.json?' + opt);
				 
				got(url.resolve('https://public-api.secure.pixiv.net/v1/', path) + '.json?' + opt , opts).then(res => {
					resolve(res.body);					
	...
	userWorks(id,opts) {
		...
		return this.AnauthGot(`users/${id}/works`, opts);
	}
